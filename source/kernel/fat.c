...
void fat_init()
{
    ...
    // (Hoping it wasn't overwritten by the stack yet).
    void* bootsect = kfindrange(0x1000)+0xC00;
    page_map(bootsect, (void*)0x7000);
    for (i = 0; i < 90; i++) BPB[i] = ((volatile char*)bootsect)[i];
    page_unmap(bootsect);
    ...
    root_start = *((unsigned short*)(BPB+BPB_RsvdSecCnt))+*((unsigned short*)(BPB+BPB_FATSz16))*BPB[BPB_NumFATs];
    root_sectors = (*((unsigned short*)(BPB+BPB_RootEntCnt))*32+*((unsigned short*)(BPB+BPB_BytsPerSec))-1)/
                   (*((unsigned short*)(BPB+BPB_BytsPerSec)));
    data_start = root_start+root_sectors;
    cluster_size = BPB[BPB_SecPerClus]*(*((unsigned short*)(BPB+BPB_BytsPerSec)));
    count_of_clusters = ((*(unsigned short*)(BPB+BPB_TotSec16) ? *(unsigned short*)(BPB+BPB_TotSec16) : *(unsigned int*)(BPB+BPB_TotSec32))-data_start)/BPB[BPB_SecPerClus];
...
int file_open(char *filename, FileStream *fs)
{
    ...
    fs->data->firstcluster = file_info.cluster_low?file_info.cluster_low:file_info.cluster_high;
...
int push_cluster(FileStream *fs)
{
    ...
    if (nextcluster >= 0xFFF8) nextcluster = 0;
...
static int read_root_file(const char* filename, int is_dir)
{
    ...
    for (i = 0; i < root_sectors; i++)
    {
        ...
        if (find_file(filename,*(unsigned short*)(BPB+BPB_BytsPerSec),is_dir)) return 1;
    }
    return 0;
}
...
static unsigned int next_cluster(int cluster)
{
    ...
    cur_fat_sec = sector;
    return *(unsigned short*)(fat+((cluster*2) % *(unsigned short*)(BPB+BPB_BytsPerSec)));
}
...
static int read_cluster(int cluster, void** buffer)
{
    ...
    for (i = 0; i < BPB[BPB_SecPerClus]; i++)
        read_sector(data_start+(cluster-2)*BPB[BPB_SecPerClus]+i, buffer);
    return 1;
}
...
static void read_sector(int sector, void** buffer)
{
    // // using CHS
    // int heads, spt;
    // sector += *((unsigned int*)BPB+BPB_HiddSec);
    // heads = *((unsigned short*)BPB+BPB_NumHeads);
    // spt = *((unsigned short*)BPB+BPB_SecPerTrk);
...
static int write_cluster(int cluster, void* buffer)
{
    ...
    if (cluster < 2) return 0;
    for (i = 0; i < BPB[BPB_SecPerClus]; i++)
        write_sector(data_start+(cluster-2)*BPB[BPB_SecPerClus]+i, buffer);
    return 1;
}
